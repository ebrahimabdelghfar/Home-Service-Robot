...
goal.target_pose.pose.position.y = -0.2;
  ...
  if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED){
    ROS_INFO("Hooray, the base to pickup");
  ...
  // Define a position and orientation for the robot to reach
  drop.target_pose.pose.position.x = 14.0-goal.target_pose.pose.position.x;
  drop.target_pose.pose.position.y = -1.0-goal.target_pose.pose.position.y;
  ...
  // Check if the robot reached its goal
  if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED){
    ROS_INFO("Hooray, the base moved to drop");
